---
name: Vita - Dark Design Overview
...
coverage:
  ...
  themeStyle: Vita - Dark
  themeStyleId: '3546271551760430036'
  themeClass: apex-theme-vita-dark
  liveRootPrimary: '#056ac8'
  standardComponents: 46
  designPages: 11
  componentAndIconPages: 60
  uniqueVitaDarkPages: 71
  requiredMenuEntries: 73
  browserVerifiedVitaDarkPages: 71
  liveComponentPagesScraped: 59
  liveComponentPagesVitaDarkVerified: 59
  liveComponentTemplateOptionPages: 31
  liveComponentEvidenceAt: '2026-06-10T18:41:21Z'
  capturedAt: '2026-06-10T18:41:21Z'
resources:
  readme: "README.md"
  manifest: "design-system.manifest.json"
  tokens: "tokens/vita-dark.tokens.json"
  css: "styles/vita-dark-open-design.css"
...
componentPatterns:
  shell:
    bodyBackground: '#252729'
    contentBackground: '#1b1d1e'
    headerBackground: '#056ac8'
    headerText: '#FFFFFF'
    sideNavigationBackground: '#2E3439'
    sideNavigationText: '#FFFFFF'
    titleBackground: '#2C2E31'
    titleText: '#FFFFFF'
    focusColor: '#056ac8'
  button:
    backgroundColor: '#494A4B'
    hoverBackgroundColor: '#494A4B'
    textColor: '#FFFFFF'
  ...
  report:
    backgroundColor: '#1B1D1E'
    borderColor: rgba(255,255,255,0.15)
    headerBackground: '#1B1D1E'
    cellBorderColor: '#323435'
    fontSize: 12px
    cellHeight: 32px
  badge:
    backgroundColor: '#FBCE4A'
    textColor: '#000000'
    rounded: 4px
    fontSize: 13px
  status:
    ...
    info: '#056AC8'
---
# Oracle APEX Universal Theme - Vita - Dark Design System

## Overview

This DESIGN.md is scoped to the **Vita - Dark** theme style of Oracle APEX Universal Theme. It is intended for Open Design import so product managers and design agents can generate APEX-style application prototypes with source-backed Vita - Dark component patterns. Keep this file independent from Iris, Redwood Light, Vita, Vita - Dark, Vita - Red, and Vita - Slate unless a task explicitly asks for a comparison.

The source evidence was collected from the public Universal Theme application after switching the right-side **Theme Style** menu to **Vita - Dark**. Verification signals: 71 unique Design, Components, and Icons paths carried `apex-theme-vita-dark`, the root primary token resolved to `#056ac8`, and 59 live component pages were scraped with 31 Template Options pages captured. Canonical component URLs remain on `oracleapex.com`; `oracleapex.cn` is cited as the fallback evidence host used for live theme switching and timeout recovery.

## Theme Style

- Theme style: **Vita - Dark**
- Theme style id: `3546271551760430036`
- Theme class: `apex-theme-vita-dark`
- Root primary token: `#056ac8`
- Visual signature: Dark APEX shell, dark canvas and surfaces, Vita blue action emphasis
...
- `README.md`: local package guide for using the Vita - Dark directory as a complete Open Design design-system package.
- `tokens/vita-dark.tokens.json`: machine-readable tokens for colors, typography, spacing, radius, shadows, sizes, and APEX class mappings.
- `styles/vita-dark-open-design.css`: compact prototype CSS that maps Vita - Dark tokens to common APEX classes such as `t-Header`, `t-Body-nav`, `t-Region`, `t-Button`, `a-IRR-table`, `a-GV-table`, `t-Card`, and `t-Icon`.
...
The `patterns/` files translate observed Universal Theme pages into generation blueprints for enterprise screens: page templates, report workspaces, editable grids, master-detail management, transaction forms, approval workflows, guided wizards, and component state matrices. Keep the output scoped to **Vita - Dark** and `.apex-theme-vita-dark`.
...
## Vita Dark Coverage Model

Open Design should treat this file as the Vita - Dark-only extraction for the Universal Theme **Design**, **Components**, and **Icons** navigation groups. The canonical URL for every listed page is `https://oracleapex.com/ords/r/apex_pm/ut/<path>`; the fallback evidence host used for live verification was `https://oracleapex.cn/ords/r/test/ut/<path>`.

### Design Menu

- **Design Overview** (`design-overview`), **Colors** (`colors`), **Navigation** (`navigation`), **Grid Layout** (`grid-layout`), **Page Templates** (`pages`), and **Theme Styles** (`theme-styles`): preserve Vita - Dark's shell, surface, navigation, and page title behavior exactly as observed.
...
## Vita Dark Token Contract

### Color

- Root primary/action token: `#056ac8`
- Body canvas: `#252729`
- Component surface: `#1b1d1e`
- Side navigation: `#2E3439` with `#FFFFFF` text
- Title area: `#2C2E31` with `#FFFFFF` text
- Report/grid cell separators: `#323435`
...
Vita - Dark screens should still look like Oracle APEX workspaces: compact header, left navigation, title/breadcrumb region, and dense content regions. Do not render Vita Dark as a black marketing dashboard; keep the APEX app shell, reports, and Template Options density.

#### Vita - Dark Navigation Menu Modes

Use one primary Navigation Menu template per application shell. Do not turn APEX navigation into a marketing navbar, oversized pill menu, or unrelated SaaS sidebar. Preserve Universal Theme classes and the dark Vita shell so generators can reproduce the observed runtime structure:
...
Neutral buttons use `#494A4B` background, `#FFFFFF` text, `rgba(255,255,255,0.075)` border, `2px` radius, `12px` font size, and `7px 11px` padding. Hot buttons use `#056AC8` with `#FFFFFF` text.

### Forms

Inputs use `#212325` background, `#FCFCFC` text, `#393D40` border, `2px` radius, and `25px 7px 5px` padding. Preserve APEX label/help/validation density and floating-label spacing.

### Regions

Regions are the main composition primitive. Use `#1B1D1E` region backgrounds, `rgba(255,255,255,0.15)` borders, `2px` radius, and `rgba(0, 0, 0, 0.1) 0px 2px 4px -2px` shadows. Template Options frequently include header visibility, heading level, body padding, maximize button, body height, icon display, and collapsibility.

### Reports, Interactive Reports, And Interactive Grids

Reports are dense. Use `#1B1D1E` grid backgrounds, `12px` cell text, `32px` grid row height, `#323435` cell separators, compact toolbar rows, and visible column headers. Interactive Grid and Interactive Report expose Template Options for Remove Borders, Show Maximize Button, Header visibility, Heading Level, item spacing, item size, stretch form fields, and label alignment.
...
## Live Vita - Dark Component Extraction Pass

A Playwright extraction pass visited the 59 unique Components pages listed below on the `oracleapex.cn` fallback host after switching the session to **Vita - Dark**. Every visited component page carried `apex-theme-vita-dark` and resolved the root primary token to `#056ac8`; no component page failed to load.
...
### Measured Vita - Dark Defaults From Live Components
...
- **Shell**: body background `#252729`, side navigation `#2E3439`, title area `#2C2E31`, and primary/focus token `#056ac8`. Do not render Vita Dark as a black marketing dashboard; keep the APEX app shell, reports, and Template Options density.
- **Buttons**: neutral `t-Button` controls are about `32px` high with `#494A4B` background, `#FFFFFF` text, 2px radius, 12px type, 16px line height, and 7px/11px padding. Hot buttons use `#056AC8` with `#FFFFFF` text.
- **Forms**: text fields use `#212325` background, `#FCFCFC` text, `#393D40` border, 2px radius, 14px type, and a `48px` floating-label height. Preserve compact APEX labels and item help/validation patterns.
...
- **Reports and grids**: Interactive Grid uses `#1B1D1E` surfaces with `rgba(255,255,255,0.15)` borders. Data cells use 12px text, 16px line height, `32px` row height, 4px/8px cell padding, and `#323435` separators. Interactive Report uses the same dense toolbar/header model.
- **Status components**: Alert regions use `#1B1D1E` surfaces, `rgba(255,255,255,0.15)` borders, 4px radius, and rgba(0, 0, 0, 0.1) 0px 2px 4px -2px shadow. Warning badges use `#FBCE4A` with `#000000` text, 4px radius, 13px type, and 4px/8px padding.
...
- Generate Vita - Dark-only Oracle APEX screens. Do not combine this file with Iris, Redwood Light, Vita, Vita - Dark, Vita - Red, or Vita - Slate unless the user explicitly asks for a multi-theme comparison.
...
All 71 unique Design, Components, and Icons page paths were visited after switching to Vita - Dark. Each visited page carried `apex-theme-vita-dark` and resolved the primary token to `#056ac8`. The later live component extraction pass revisited 59 unique Components pages in the same browser session and captured page anatomy, live demo selectors, Template Option families, top APEX/UT classes, and representative computed styles.
