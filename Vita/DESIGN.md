...
#### Vita Navigation Menu Modes

Use one primary Navigation Menu template per application shell. Do not turn APEX navigation into a marketing navbar, oversized pill menu, or unrelated SaaS sidebar. Preserve the Universal Theme classes and densities so generators can reproduce the observed runtime structure:

- **Menu Bar / Top Navigation Menu**: Use `apex-top-nav t-PageBody--topNav` with `t-Header`, `t-Header-branding`, `t-Header-nav`, and `t-Header-nav-list js-tabLike a-MenuBar a-MenuBar--overflow a-MenuBar--tabs`. Vita shows a 48px blue branding row followed by a 48px dark menu row (`#2e3439`). Top-level `a-MenuBar-item` entries are horizontal, 14px text, white labels, `a-Menu--split` for items with submenus, and `a-Menu--current` for the current route. Split entries keep label padding tight and add a separate 32px `a-Menu-subMenuCol` cell with `icon-menu-split-drop-down`. Focus/hover should darken to near `#171a1d`; submenus use APEX `a-Menu` surfaces and optional callouts rather than custom dropdown cards.
- **Tabs / Top Navigation Tabs**: Use `apex-top-nav t-PageBody--topNav` with `t-NavTabs t-NavTabs--inlineLabels-lg t-NavTabs--displayLabels-sm` inside `t-Header-nav`. Vita renders the blue branding row at 48px and the tabs row at 40px on the dark navigation background. `t-NavTabs-item` entries divide available width, `t-NavTabs-link` uses 14px white labels with 8px/12px padding, optional `t-Icon fa-*` icons use a 22px icon box before the label, and the active/current tab uses the darker active background. Keep this mode for simple applications with about six or fewer primary destinations; on narrow/mobile layouts it can move to the bottom according to Universal Theme behavior.
- **Mega Menu / Top Navigation Mega Menu**: Use `apex-top-nav` with no persistent top nav list and expose the header menu button as `t-Button--headerTree t-Button--megaMenuToggle`. When open, the menu is `t-MegaMenu a-Menu` positioned from the header button, with an `a-Menu-content t-MegaMenu-container` white panel, 1px subtle border, and `0 12px 24px -12px rgba(0,0,0,.3)` shadow. Top-level groups use `t-MegaMenu-list--top` as a grid, `t-MegaMenu-item--top t-MegaMenu-item--hasSub` columns with 16px padding, blue 14px bold `t-MegaMenu-label` headings, and 32px sub-item rows. Use this for broad applications where users need many destinations visible at once.
- **Side Tree Navigation**: Use `apex-side-nav t-PageBody--leftNav` with `t-Body-nav` and `a-TreeView t-TreeNav js-defaultCollapsed t-TreeNav--styleA`. Vita renders the expanded side panel at 240px wide, dark `#2e3439`, sticky below the 48px header, and uses 44px top-level rows. APEX standard side navigation is collapsible: expose the header control as `t_Button_navControl t-Button--headerTree`, toggle `js-navExpanded` / `js-navCollapsed` on the page body, keep the expanded body grid at `240px minmax(0,1fr)`, and collapse the nav grid column to `0px` while TreeView content compresses to a 16px icon strip with transparent labels. Each node has `a-TreeView-row`, `a-TreeView-content`, optional sibling `a-TreeView-toggle`, and `a-TreeView-label`; the content grid is a 28px icon column plus label column, while expandable nodes reserve a separate 32px toggle area. Top-level items carry `a-TreeView-node--topLevel`, leaf nodes carry `a-TreeView-node--leaf`, expandable nodes carry `is-expandable` or `is-collapsible`, child rows use 40px height with 12px labels, and the current page is marked on `a-TreeView-row` with `is-selected is-current--top` so the dark selected background spans the full panel. Indent nested items with tree structure, not arbitrary padding.
- **Navigation Bar**: Keep `t-NavigationBar` in the header end area for utilities such as authentication, help, feedback, version, or theme style. It is not a replacement for the main Navigation Menu and should remain compact, icon/text based, and at most two levels deep.

The canonical evidence page is `https://oracleapex.com/ords/r/apex_pm/ut/navigation`; live Vita verification used the fallback app and preview pages `1120` (Menu Bar), `1121` (Tabs), `1122` (Mega Menu), and `1123` (Side Tree Navigation) on `https://oracleapex.cn/ords/r/test/ut/navigation` when the primary host timed out.
...
List and navigation components should remain utilitarian. Links List supports badges, right arrows, icon display, and action styling. Menu Bar uses APEX `a-MenuBar` and `a-Menu` behavior, not generic dropdown markup. Top Navigation Tabs use `t-NavTabs`, while region-level Tabs Container remains `t-Tabs`; do not confuse those two tab systems. Tabs Container supports remembered active tab, icon placement, fill labels, simple/pill style, and small/large sizes. Wizard supports vertical orientation, clickable steps, and label display modes.
